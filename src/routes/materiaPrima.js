...
router.post('/regCompra/:id', async (req, res)=>{ // Registra la compra de alguna de las materias primas previamente registradas (nombre y precio)
    const { id } = req.params;
    // var dateUpdate = new Date().toISOString();
    const { lote, cantidad , nameProveedor, claveProductor, dateCompra, costales, claveCostales, quienEntrego, quienRecibio } = req.body;

    const materiaPrima = {lote, cantidad , nameProveedor, claveProductor, dateCompra, costales, claveCostales, quienEntrego, quienRecibio };
