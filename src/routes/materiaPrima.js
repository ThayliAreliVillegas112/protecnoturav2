...
router.get('/stock', async (req, res) => { //OBTIENE LA CANTIDAD QUE SE REGISTRA EN LA COMPRA
    let listcom = await pool.query('SELECT * FROM registerCompra');
    
    res.json({
        status: 200,
        message: "Se ha listado correctamente las cantidades ",
        listcom: listcom
    });
    console.log(listcom);
})
