...
-- Schema mydb
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `protecnotura` DEFAULT CHARACTER SET utf8 ;
USE `protecnotura` ;
...
CREATE TABLE IF NOT EXISTS `protecnotura`.`materiaPrima` (
  ...
  `nameProveedor` VARCHAR(45) NULL,
  `claveProductor` VARCHAR(45) NULL,
  `dateCompra` VARCHAR(45) NULL,
  `costales` INT NULL,
  `claveCostales` VARCHAR(45) NULL,
  `quienEntrego` VARCHAR(45) NULL,
  `quienRecibio` VARCHAR(45) NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `protecnotura`.`harina` (
  ...
  `cantidad` DOUBLE NULL,
  `cantidadHarina` DOUBLE NULL,
  `dateRegistro` VARCHAR(45) NULL,
  `materiaPrima_id` INT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_harina_materiaPrima1_idx` (`materiaPrima_id` ASC),
  CONSTRAINT `fk_harina_materiaPrima1`
...
CREATE TABLE IF NOT EXISTS `protecnotura`.`detallesPedidoP` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_detallesPedidoP_pedido_idx` (`pedido_id` ASC),
  INDEX `fk_detallesPedidoP_product1_idx` (`product_id` ASC),
  CONSTRAINT `fk_detallesPedidoP_pedido`
    FOREIGN KEY (`pedido_id`)
    REFERENCES `protecnotura`.`pedido` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_detallesPedidoP_product1`
